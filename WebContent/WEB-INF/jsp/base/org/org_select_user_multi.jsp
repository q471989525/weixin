...
<html>
  <head>
	...
	//角色查找用户
  	...
  	function confirmSelect(){
  		...
  		if(users.length>0 && users.length<=limitSize){
	  		//window.returnValue = users;
  			window.parent.selectedMultiUsers(users);
  		}else if(users.length>limitSize){
  			$.messager.alert("警告", "最多只能选择"+limitSize+"个用户！","warning");
  			return;
  		}else{
    ...
    function clickRow(rowIndex, rowData){
    	...
    	if(users.length>0 && users.length<limitSize){
    		var addFlag = true;
    		for (var i = 0; i < users.length; i++) {
  				if(users[i].userid == rowData.userid){addFlag = false; break;}
  			}
    		if(addFlag) users.push(rowData);
	    	window.parent.selectedMultiUsers(users);
    	}else if(users.length==0){
    		var rows = new Array();
    		rows.push(rowData);
	    	window.parent.selectedMultiUsers(rows);
    	}else{
    		$.messager.alert("警告", "最多只能选择"+limitSize+"个用户！","warning");
  			return;
    	}
  ...
  <body class="easyui-layout">
  	<div data-options="region:'north'" title="用户选择(最多可选${limitSize}个)" style="height:60px;padding: 2px;">
  		<center>
	  		<form id="queryForm" onkeypress="onFormKeyPress(event)" onsubmit="return false;">
	  			姓名：<input id="username" value="" type="text" style="width: 145px;">&nbsp;&nbsp;
  	...
  	<div data-options="region:'center',border:false">
  		<table id="userDg" class="easyui-datagrid" title="用户列表" data-options="rownumbers:true,fit:true,onDblClickRow:clickRow,toolbar:'#tb'">
		...
		<div id="tb" style="height:auto">
			<a class="easyui-linkbutton" data-options="iconCls:'icon-tick-button',plain:true" onclick="searchAllUser()">全选</a>|
			<a class="easyui-linkbutton" data-options="iconCls:'icon-cross-button',plain:true" onclick="clearAllUser()">反选</a>|
