...
<html>
  	...
  	<script type="text/javascript">
	  	$(document).ready(function(){
	  		...
	  		$("#businessForm").validate({
				rules: {
					businessName: {required:true,maxlength:80},
	  	        ...
	  	        success:function (data){
	            	if(data.state == "success"){
		                $.messager.alert("提示", "保存成功", "info",function(){
		                	try {
				            	opener.searchBusiness(); //调list页面查询方法
							} catch (e) { }
			   	    		window.close();
		   	        	});
	            	}else{
			...
			$("#file_upload").uploadify({
		        ...
		        formData:{"attachmentId":"${business.attachmentId}"},
			...
			UE.getEditor("detailExplain"); //初始化,放到最后不影响附件初始化
  ...
  <body class="easyui-layout">
  	<form id="businessForm" action="${pageContext.request.contextPath}/businessopportunity/save.do" method="post">
  	<div data-options="region:'center'">
  		<div class="easyui-tabs" data-options="fit:true,border:false">
			<div title="商机信息">
				<input type="hidden" id="resourceid" name="resourceid" value="${business.resourceid }">
			  	<input type="hidden" id="createId" name="createId" value="${business.createId }">
			  	<input type="hidden" id="creator" name="creator" value="${business.creator }">
			  	<input type="hidden" id="createDept" name="createDept" value="${business.createDept }">
			  	<input type="hidden" id="createTime" name="createTime" value="<fmt:formatDate value="${business.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/>">
			  	<input type="hidden" id="deleteFlag" name="deleteFlag" value="${business.deleteFlag }">
				<table style="width:100%;" border="0" cellspacing="0" cellpadding="0" class="formtable">
					<tbody>
				    	<tr>
				        	<td class="tdTitle" style="width: 15%;">商机名称：</td>
					        <td colspan="3">
					        	<input type="text" id="businessName" name="businessName" value="${business.businessName }" style="width: 80%;">
					    	</td>
					    </tr>
					    <tr>  
					    	<td class="tdTitle" style="width: 15%;">项目名称：</td>
					        <td style="width: 30%;">
					        	<input type="hidden" id="projectId" name="projectId" value="${business.projectId }">
					            <input type="text" id="projectName" name="projectName" value="${business.projectName }">
					        ...
					        <td>
					          	<input type="text" id="customerName" name="customerName" value="${business.customerName }">
					        </td>
					    </tr>
					    <tr>
					    	<td class="tdTitle">业务类型：</td>
					        <td>
					        	<app:select name="businessType" dictionaryCode="D_Business_Type" value="${business.businessType}"/>
					        </td>
					        <td class="tdTitle">商机来源：</td>
					        <td>
					          	<app:select name="businessSource" dictionaryCode="D_Business_Source" value="${business.businessSource}"/>
					        </td>
					    </tr>
					    <tr>
					        <td class="tdTitle">商机状态：</td>
					        <td colspan="3">
					           	<app:select name="businessStatus" dictionaryCode="D_Business_Status" value="${business.businessStatus}"/>
					        </td>
					    </tr>
					    <tr>  
					        <td class="tdTitle">附件：</td>
					        <td colspan="3">
					           	<input type="hidden" id="attachmentId" name="attachmentId" value="${business.attachmentId }">
					     ...
					     <tr>  
					        <td class="tdTitle">详细说明：</td>
					        <td colspan="3">
					           	<textarea rows="4" style="width: 650px;" id="detailExplain" name="detailExplain">${business.detailExplain}</textarea>
					        </td>
					     </tr>
					</tbody>
				</table>
	...
	<div data-options="region:'south'" style="height:30px; padding-top:1px; overflow: hidden; background-color: rgb(244, 244, 244)" align="center">
		<app:btnAuth btnUrl="/businessopportunity/save.do">
